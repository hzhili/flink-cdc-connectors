...
public class SqlServerCatalogFactory implements CatalogFactory {
    ...
    public Catalog createCatalog(Context context) {
        ...
        SqlServerJdbcConfiguration.Builder jdbcConfigBuilder =
                SqlServerJdbcConfiguration.copy(configuration.subset(DEBEZIUM_OPTIONS_PREFIX, true));
        String hostname = configuration.getString(HOSTNAME.key());
        jdbcConfigBuilder
                .with(SqlServerConnectorConfig.HOSTNAME, hostname)
                .with(SqlServerConnectorConfig.PORT, configuration.getInteger(PORT.key(), PORT.defaultValue()));
        configuration
                .edit()
        ...
        SqlServerJdbcConfiguration jdbcConfiguration =
                jdbcConfigBuilder
                        ...
                        .with(SqlServerConnectorConfig.USER, configuration.getString(USERNAME.key()))
                        .with(SqlServerConnectorConfig.PASSWORD, configuration.getString(PASSWORD.key()))
                        .build();
        return new SqlServerCatalog(context.getName(), databaseName, configuration, jdbcConfiguration);
    }
