...
public class SqlServerConnection extends JdbcConnection {
    ...
    private static final String AGENT_RUNNING = "Running.";
    ...
    //            .withDefault(
    //                    "SELECT CASE WHEN dss.[status]=4 THEN 1 ELSE 0 END AS isRunning FROM
    // [#db].sys.dm_server_services dss WHERE dss.[servicename] LIKE N'SQL Server Agent (%';");
    ...
    public boolean isAgentRunning(String databaseName) throws SQLException {
        ...
        //                singleResultMapper(rs -> rs.getBoolean(1), "SQL Server Agent running
        // status query must return exactly one value"));
        return queryAndMap(
                query,
                rs -> {
                    if (rs.next()) {
                        return rs.getString(1).equalsIgnoreCase(AGENT_RUNNING);
                    }
                    return false;
                });
    }
